...
(CIs).\footnote{Note that the CIs come from fitting quantile regressions, not OLS, as the confidence intervals vary at each $\tau$.}
...
treating it as a
constant.\footnote{See Figure 4 of Koenker and Hallock (2001) where both CIs are reported.}
Therefore, the test holds valid only if we assume the point estimate of
the regression slope to be the true parameter, an unreliable assumption
for proper
...
Koenker, R., \& Hallock K. F. (2001). Quantile Regression. \emph{Journal
of Economic Prespectives}, 15(4), 143-156.
